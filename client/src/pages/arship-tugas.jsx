...
export default function ArshipTugas() {
    ...
    return (<main className="min-h-screen bg-gray-50 dark:bg-gray-900 py-6 sm:py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="text-center mb-8 sm:mb-12">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="grid sm:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto">
          {tasks.map(function (task, index) { return (<motion.div key={task.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 + index * 0.1, ease: "easeOut" }} whileHover={{ y: -8, scale: 1.02 }} className="cursor-pointer" onClick={function () { return handleTaskClick(task); }}>
              <Card className="overflow-hidden hover:shadow-xl transition-all duration-500 group border-gray-200 hover:border-blue-200">
                <div className="relative overflow-hidden">
                  <img src={task.image} alt={task.title} className="w-full h-40 sm:h-48 object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy"/>
                ...
                <CardContent className="p-5 sm:p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <motion.div whileHover={{ rotate: 15, scale: 1.2 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
