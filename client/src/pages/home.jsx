...
export default function Home() {
    ...
    return (<main className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Hero Section */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="text-center mb-16">
          <div className="mb-8 rounded-2xl overflow-hidden shadow-2xl max-w-4xl mx-auto">
            <img src={PraktekImg} alt="Praktek Mandiri drg. Danny Hanggono" className="w-full h-auto max-h-[400px] sm:max-h-[450px] md:max-h-[500px] object-cover object-center transform hover:scale-102 transition-transform duration-500"/>
          </div>
        ...
        {/* Features Grid */}
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2, ease: "easeOut" }} className="grid md:grid-cols-3 gap-8 mb-16">
          {features.map(function (feature, index) { return (<motion.div key={feature.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{
                duration: 0.6,
                delay: 0.3 + index * 0.1,
                ease: "easeOut"
            }} whileHover={{
              ...
              <Card className="h-full hover:shadow-lg transition-all duration-500 border-opacity-70 hover:border-blue-200">
                <CardContent className="p-6">
                  <motion.div className="text-blue-600 text-3xl mb-4" whileHover={{ scale: 1.1 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
